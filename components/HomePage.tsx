...
type StatsData = {
  ...
  membersPeriod: { uz: string; en: string };
...
const DEFAULT_STATS: StatsData = {
  ...
  sport: "12",
  presentations: "4",
  membersPeriod: { uz: "Jamoada", en: "On the team" },
  period: { uz: "1 oy ichida", en: "In 1 month" },
};
...
function ArrowLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link href={href} className="inline-flex items-center gap-1.5 text-sm font-semibold text-gold-ink transition-colors hover:text-gold dark:text-gold-light dark:hover:text-gold">
      {children}
...
export default function HomePage({
  ...
  const statItems = [
    { value: s.members, key: "home.stats.members", period: locale === "uz" ? s.membersPeriod.uz : s.membersPeriod.en },
    { value: s.meetings, key: "home.stats.meetings", period: locale === "uz" ? s.period.uz : s.period.en },
    { value: s.sport, key: "home.stats.sport", period: locale === "uz" ? s.period.uz : s.period.en },
    { value: s.presentations, key: "home.stats.presentations", period: locale === "uz" ? s.period.uz : s.period.en },
  ];
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-8 lg:hidden">
              {statItems.map((stat, i) => (
                <Reveal key={stat.key} delay={i * 80}>
                  <StatBlock value={stat.value} label={t(stat.key)} period={stat.period} />
                </Reveal>
              ))}
            </div>
          ...
          <div className="grid gap-6 lg:grid-cols-2 lg:gap-16">
            <Reveal>
              ...
              <h2 className="mt-3 font-display text-4xl leading-tight md:text-5xl">
                ...
                <span className="text-gold-ink dark:text-gold-light">{t("home.about.titleGold")}</span>
              </h2>
            </Reveal>
        ...
        <section className="mx-auto max-w-6xl px-4 py-10 md:px-6 lg:pb-16">
          <Reveal><SectionLabel>{t("home.directions.overline")}</SectionLabel></Reveal>
        ...
        {nextMeeting && (
          <section className="mx-auto mt-[50px] max-w-6xl px-4 pb-10 md:px-6 lg:pb-16">
            <Reveal><NextMeetingCard meeting={nextMeeting} className="mx-auto max-w-2xl" /></Reveal>
          </section>
        )}
        <TeamArc members={members} />
        <OneWaySection />
        <section className="relative overflow-hidden bg-navy-grad">
          <div className="mx-auto max-w-6xl px-4 py-14 text-center md:px-6 lg:py-20">
            <Reveal>
              ...
              <Button variant="gold" size="lg" href="/join" className="mt-7">{t("home.cta.button")}</Button>
            </Reveal>
          </div>
        </section>
      </main>
